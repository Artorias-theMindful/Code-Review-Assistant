...
def get_code_review_from_ollama(patch_text: str) -> dict:
    ...
    prompt = (
        "You are an expert code reviewer. Given the following git patch, produce concise sections:\n"
    ...
    try:
        ...
        resp = requests.post(endpoint, json={"model": model, "prompt": prompt, "stream": False, "options": { "num_predict": 1024 }})
        print(resp.json())
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Failed to contact Ollama: {e}")

    if resp.status_code != 200:
        raise HTTPException(status_code=502, detail=f"Ollama API error: {resp.status_code} {resp.text}")
...
async def upload_patch(
    ...
    if file is None and patch_text is None:
        raise HTTPException(status_code=400, detail="Provide either a file upload or patch_text form field.")
    ...
    if not patch_text or not patch_text.strip():
        raise HTTPException(status_code=400, detail="Patch content cannot be empty.")
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Error generating code review: {e}")
...
async def upload_patch_json(payload: dict):
    ...
    if not isinstance(patch_text, str) or not patch_text.strip():
        raise HTTPException(status_code=400, detail="JSON body must include a non-empty patch_text string.")
